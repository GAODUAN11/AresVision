...
class InformerBackbone(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, n_vars, d_model, n_heads, e_layers, d_ff, dropout):
        ...
        self.encoder_layers = nn.ModuleList([InformerEncoderLayer(d_model=d_model, n_heads=n_heads, d_ff=d_ff, dropout=dropout) for _ in range(e_layers)])
        ...
        self.head = nn.Sequential(nn.Flatten(start_dim=1), nn.Linear(self.final_len * d_model, pred_len * n_vars))
...
class GridPointInformerO3(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, lat_size, lon_size, use_phase_warp, d_model, n_heads, e_layers, d_ff, dropout):
        ...
        if use_phase_warp:
            ...
            n_vars = 5
        self.backbone = InformerBackbone(seq_len=seq_len, pred_len=pred_len, n_vars=n_vars, d_model=d_model, n_heads=n_heads, e_layers=e_layers, d_ff=d_ff, dropout=dropout)
...
def main():
    ...
    for seed in RUN_SEEDS:
        ...
        raw_metrics = train_and_evaluate(label=f'Informer_Raw_seed{seed}', use_phase_warp=False, train_loader=train_loader, test_loader=test_loader, device=device, y_std=y_std, y_mean=y_mean, lat_size=lat_size, lon_size=lon_size, window=window, horizon=horizon, d_model=d_model, n_heads=n_heads, e_layers=e_layers, d_ff=d_ff, dropout=dropout, epochs=epochs, learning_rate=learning_rate, early_stopping_patience=early_stopping_patience, base_dir=base_dir)
        raw_seed_metrics.append(raw_metrics)
        set_experiment_seed(seed)
        phase_metrics = train_and_evaluate(label=f'Informer_PhaseWarp_seed{seed}', use_phase_warp=True, train_loader=train_loader, test_loader=test_loader, device=device, y_std=y_std, y_mean=y_mean, lat_size=lat_size, lon_size=lon_size, window=window, horizon=horizon, d_model=d_model, n_heads=n_heads, e_layers=e_layers, d_ff=d_ff, dropout=dropout, epochs=epochs, learning_rate=learning_rate, early_stopping_patience=early_stopping_patience, base_dir=base_dir)
    ...
    print('\n[Comparison Summary]')
    print_seed_summary('Informer_Raw', raw_summary)
    print_seed_summary('Informer_PhaseWarp', phase_summary)
