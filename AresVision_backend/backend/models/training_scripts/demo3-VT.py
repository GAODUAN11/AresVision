...
sys.stdout = Logger(os.path.join(base_dir, "models", "training_logs", "VT.txt"))
...
active_vars = [c for c in "VT" if c in mcd_vars_map]
...
# 5. 构建 X, y 数据集
# ========================================
X_raw = np.stack([y_raw, vars_dict['v'], vars_dict['temp']], axis=-1)
...
save_path = args.output_path or os.path.join(base_dir, "models", "training_results", "predrnn_VT.pth")
