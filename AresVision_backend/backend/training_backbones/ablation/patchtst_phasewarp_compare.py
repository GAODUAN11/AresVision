...
class GridPointPatchTSTO3(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, lat_size, lon_size, use_phase_warp, patch_len, stride, d_model, n_heads, e_layers, d_ff, dropout):
        super().__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
        ...
        if use_phase_warp:
            ...
            n_vars = 5
        self.backbone = PatchTSTBackbone(seq_len=seq_len, pred_len=pred_len, n_vars=n_vars, patch_len=patch_len, stride=stride, d_model=d_model, n_heads=n_heads, e_layers=e_layers, d_ff=d_ff, dropout=dropout)
...
def train_and_evaluate(label, use_phase_warp, train_loader, test_loader, device, y_std, y_mean, lat_size, lon_size, window, horizon, patch_len, stride, d_model, n_heads, e_layers, d_ff, dropout, epochs, learning_rate, early_stopping_patience, base_dir):
    """
    ...
    """
    print(f'\n[Experiment] {label}')
    model = GridPointPatchTSTO3(seq_len=window, pred_len=horizon, lat_size=lat_size, lon_size=lon_size, use_phase_warp=use_phase_warp, patch_len=patch_len, stride=stride, d_model=d_model, n_heads=n_heads, e_layers=e_layers, d_ff=d_ff, dropout=dropout).to(device)
...
def main():
    ...
    sys.stdout = Logger(os.path.join(base_dir, 'models', 'training_logs', 'PatchTST_PhaseWarp_Compare.txt'))
    ...
    for seed in RUN_SEEDS:
        ...
        raw_metrics = train_and_evaluate(label=f'PatchTST_Raw_seed{seed}', use_phase_warp=False, train_loader=train_loader, test_loader=test_loader, device=device, y_std=y_std, y_mean=y_mean, lat_size=lat_size, lon_size=lon_size, window=window, horizon=horizon, patch_len=patch_len, stride=stride, d_model=d_model, n_heads=n_heads, e_layers=e_layers, d_ff=d_ff, dropout=dropout, epochs=epochs, learning_rate=learning_rate, early_stopping_patience=early_stopping_patience, base_dir=base_dir)
        raw_seed_metrics.append(raw_metrics)
        set_experiment_seed(seed)
        phase_metrics = train_and_evaluate(label=f'PatchTST_PhaseWarp_seed{seed}', use_phase_warp=True, train_loader=train_loader, test_loader=test_loader, device=device, y_std=y_std, y_mean=y_mean, lat_size=lat_size, lon_size=lon_size, window=window, horizon=horizon, patch_len=patch_len, stride=stride, d_model=d_model, n_heads=n_heads, e_layers=e_layers, d_ff=d_ff, dropout=dropout, epochs=epochs, learning_rate=learning_rate, early_stopping_patience=early_stopping_patience, base_dir=base_dir)
    ...
    print('\n[Comparison Summary]')
    print_seed_summary('PatchTST_Raw', raw_summary)
    print_seed_summary('PatchTST_PhaseWarp', phase_summary)
