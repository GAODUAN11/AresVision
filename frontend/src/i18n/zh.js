...
const zh = {
  ...
  explore: {
    ...
    stepUploadDesc: '上传 MCD、OpenMARS 或 NOMAD 的 `.nc` 文件，系统先识别类型、火星年、Ls 范围和网格结构。',
    ...
    defaultDataset: {
      ...
      sourceMode: {
        default: '官方默认数据源',
        personal_full_year: '个人全年数据源',
        personal_mcd_plus_system_openmars: '系统 OpenMARS + 个人 MCD',
      },
    },
    ...
    upload: {
      ...
      loginBtn: '登录',
      title: '上传火星观测数据',
      subtitle: '支持 MCD、OpenMARS、NOMAD (.nc) 格式。系统会自动校验数据类型、火星年、Ls 范围、变量和网格分辨率，用于数据总览可视化。',
    ...
    myData: {
      ...
      ingestDesc: '这里管理用于数据总览可视化的原始上传数据。训练与预测使用服务器后台维护的数据，不再使用用户上传的个人融合数据。',
      ingestCheck1: '识别类型',
      ingestCheck2: '校验网格',
      ingestCheck3: '可用于数据总览',
      statUploads: '上传记录',
      statUploadsDesc: '你当前上传到平台的全部数据文件。',
      statUsable: '当前可用',
      statUsableDesc: '可以在数据总览中选择使用的数据。',
      statYears: '激活火星年',
      statYearsDesc: '当前个人或混合数据源已支持的火星年数量。',
      currentSourceTitle: '当前个人数据源状态',
      currentSourceBuild: '构建状态',
      currentSourceBuildDesc: '上传后系统会在后台整理并构建个人数据源缓存。',
      currentSourceEffective: '当前生效数据源',
      currentSourceEffectiveDesc: '其他页面切换到个人模式后，实际使用的是哪种数据源模式。',
      currentSourceYears: '已接入火星年',
      currentSourceYearsDesc: '当前真正可以在其他页面被个人模式调用的火星年数量。',
      currentSourceEmpty: '当前还没有可用的个人数据源火星年。',
      currentSourceHint: '上传通过后，系统会自动尝试把数据接入个人数据源；若条件不足，则继续回退到默认系统数据源。',
      currentSourceUseHint: '当数据概览、预测分析或模型训练切换到个人模式时，会使用这里显示的生效数据源。',
      cardType: '类型',
      cardMarsYear: '火星年',
      cardLs: 'Ls',
      cardGrid: '网格',
      ...
      datasetStateDesc: {
        ...
        rejected: '该文件未通过公共审核，不能并入平台默认数据集；但若已接入个人链路，上传者本人仍可在个人模式中继续使用。',
        rejectedMixed: '该文件未通过公共审核，不能并入平台默认数据集；但你的个人 MCD 仍可继续参与混合数据源，供上传者本人在个人模式中使用。',
        failed: '后台在构建个人数据源时失败，当前仍会回退到默认系统数据源。',
        building: '上传已通过，系统正在构建个人数据源，完成后其他页面即可尝试切换使用。',
        personal: '该数据已经作为个人数据源的一部分，可以在数据概览、分析、预测和训练中被个人模式使用。',
        mixed: '该数据已经进入混合模式，系统会使用默认 OpenMARS 与你的 MCD 共同支撑其他页面。',
        fallback: '当前混合模式下使用的是系统 OpenMARS，这份 OpenMARS 文件本身没有被个人模式采用。',
        inactive: '该数据目前没有进入生效的数据源链路，因此其他页面不会使用它。',
      },
      usage: {
        visual: '数据概览',
        analysis: '数据分析',
  ...
  predict: {
    ...
    avgR2: '平均决定系数',
    shapleyTitle: '边际贡献分析',
    shapleyDesc: '基于博弈论的边际分配方法，计算加入各气象特征后对模型整体预测性能的贡献。',
    shapleyGenerateBtn: '分析特征贡献',
    ...
    shapley: {
      ...
      gradientTab: '梯度归因',
      marginalTab: '组合边际',
      importanceTitle: '特征重要性（平均绝对归因值）',
      swarmTitle: '摘要蜂群图（全球分布）',
      marginalAnalysisTitle: '边际贡献分析',
      featureValue: '特征数值',
      lowValue: '低值',
      highValue: '高值',
      meanShap: '平均绝对归因值',
      impactOnPred: '归因值（对预测的影响）',
      avgContribution: '{metric} 的平均边际贡献',
      mathPrinciple: '数学原理',
      mathDesc: '利用所有 2ⁿ 种变量组合下的边际提升加权平均计算得出。这代表了每个输入特征对模型 {metric} 性能提升的公平分配份额。',
      metrics: {
        r2: '决定系数（R²）',
        rmse: '均方根误差（RMSE）',
        mae: '平均绝对误差（MAE）',
      ...
      cacheMissing: '边际缓存缺失',
      cacheMissingDesc: '该模式需要 32 组模型对比。若加载过慢，请先在侧边栏点击“遍历组合”生成缓存。',
      footerNote: '* 归因值表示每个特征对模型预测场空间均值的加性贡献。\n* 数据点从全量测试集（MY28）中随机下采样，以保证渲染性能。',
      computing: '归因计算中...',
      waterfallTitle: '瀑布归因分析',
      waterfallDesc: '展示从基准值到最终预测值的逐步归因过程。',
      baseValue: '基准值',
      totalValue: '预测值',
      inhibitory: '抑制作用',
      promotional: '促进作用',
    ...
    shapleyMarginalBtn: '🧬 组合边际分析',
    shapleyGeneratingBtn: '正在计算全局归因...',
    barChart: {
      analytics: '轨道分析',
      spectrum: '性能频谱',
      topModel: '最优模型',
    ...
    shapley: {
      ...
      gradientTab: '梯度归因',
      marginalTab: '组合边际',
      cacheMissing: '注意：缓存未命中',
      cacheMissingDesc: '当前指标的边际贡献需要重新遍历计算流程。',
      importanceTitle: '特征重要性',
      meanShap: '平均归因值（对预测强度的影响）',
      swarmTitle: '摘要蜂群图',
      impactOnPred: '对预测值的影响',
      marginalAnalysisTitle: '特征边际贡献',
      avgContribution: ({ metric }) => `平均边际贡献 (${metric})`,
      mathPrinciple: '数学原理',
      mathDesc: ({ metric }) => `基于博弈论的分配方法，计算特征对模型 ${metric} 的独立边际贡献。`,
      mathDescNote: {
        higher: ({ metric }) => `正值代表该特征提升了 ${metric}。`,
        lower: ({ metric }) => `负值代表该特征降低了 ${metric}。`,
      },
      lowValue: '特征低值',
      highValue: '特征高值',
      footerNote: '数据基于测试集全样本计算 • 红蓝点表示特征原始数值的高低\n该分析用于提升模型预测逻辑的全局透明度',
      featureValue: '特征数值',
    },
  },
