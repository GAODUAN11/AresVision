...
class TSMixerResBlock(nn.Module):
    ...
    def __init__(self, seq_len, n_vars, hidden_dim, dropout):
        super().__init__()
        self.temporal = nn.Sequential(nn.Linear(seq_len, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, seq_len), nn.Dropout(dropout))
        self.channel = nn.Sequential(nn.Linear(n_vars, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, n_vars), nn.Dropout(dropout))
...
class TSMixerBackbone(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, n_vars, hidden_dim, e_layers, dropout):
        super().__init__()
        self.layers = nn.ModuleList([TSMixerResBlock(seq_len=seq_len, n_vars=n_vars, hidden_dim=hidden_dim, dropout=dropout) for _ in range(e_layers)])
...
class GridPointTSMixerO3(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, lat_size, lon_size, use_phase_warp, hidden_dim, e_layers, dropout):
        ...
        if use_phase_warp:
            ...
            n_vars = 5
        self.backbone = TSMixerBackbone(seq_len=seq_len, pred_len=pred_len, n_vars=n_vars, hidden_dim=hidden_dim, e_layers=e_layers, dropout=dropout)
...
def train_and_evaluate(label, use_phase_warp, train_loader, test_loader, device, y_std, y_mean, lat_size, lon_size, window, horizon, hidden_dim, e_layers, dropout, epochs, learning_rate, early_stopping_patience, base_dir):
    """
    训练一组 TSMixer 实验，并返回评估指标。
    受控变量只有一个：是否启用 phase-warp 前端。
    """
    print(f'\n[Experiment] {label}')
    model = GridPointTSMixerO3(seq_len=window, pred_len=horizon, lat_size=lat_size, lon_size=lon_size, use_phase_warp=use_phase_warp, hidden_dim=hidden_dim, e_layers=e_layers, dropout=dropout).to(device)
...
def main():
    ...
    sys.stdout = Logger(os.path.join(base_dir, 'models', 'training_logs', 'TSMixer_PhaseWarp_Compare.txt'))
    ...
    for seed in RUN_SEEDS:
        ...
        raw_metrics = train_and_evaluate(label=f'TSMixer_Raw_seed{seed}', use_phase_warp=False, train_loader=train_loader, test_loader=test_loader, device=device, y_std=y_std, y_mean=y_mean, lat_size=lat_size, lon_size=lon_size, window=window, horizon=horizon, hidden_dim=hidden_dim, e_layers=e_layers, dropout=dropout, epochs=epochs, learning_rate=learning_rate, early_stopping_patience=early_stopping_patience, base_dir=base_dir)
        raw_seed_metrics.append(raw_metrics)
        set_experiment_seed(seed)
        phase_metrics = train_and_evaluate(label=f'TSMixer_PhaseWarp_seed{seed}', use_phase_warp=True, train_loader=train_loader, test_loader=test_loader, device=device, y_std=y_std, y_mean=y_mean, lat_size=lat_size, lon_size=lon_size, window=window, horizon=horizon, hidden_dim=hidden_dim, e_layers=e_layers, dropout=dropout, epochs=epochs, learning_rate=learning_rate, early_stopping_patience=early_stopping_patience, base_dir=base_dir)
    ...
    print('\n[Comparison Summary]')
    print_seed_summary('TSMixer_Raw', raw_summary)
    print_seed_summary('TSMixer_PhaseWarp', phase_summary)
