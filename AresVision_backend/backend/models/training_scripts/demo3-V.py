...
sys.stdout = Logger(os.path.join(base_dir, "models", "training_logs", "V.txt"))
...
active_vars = [c for c in "V" if c in mcd_vars_map]
...
# 5. 构建 X, y 数据集
# ========================================
X_raw = np.stack([y_raw, vars_dict['v']], axis=-1)
...
save_path = args.output_path or os.path.join(base_dir, "models", "training_results", "predrnn_V.pth")
