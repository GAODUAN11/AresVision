...
class NBeatsBackbone(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, n_vars, stack_count, blocks_per_stack, hidden_dim, n_layers, dropout):
        ...
        self.blocks = nn.ModuleList([NBeatsBlock(input_size=input_size, forecast_size=forecast_size, hidden_dim=hidden_dim, n_layers=n_layers, dropout=dropout) for _ in range(total_blocks)])
    ...
    def forward(self, x):
        ...
        forecast = torch.zeros(x.size(0), self.pred_len * self.n_vars, device=x.device, dtype=x.dtype)
...
class GridPointNBeatsO3(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, lat_size, lon_size, use_phase_warp, stack_count, blocks_per_stack, hidden_dim, n_layers, dropout):
        super().__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
        ...
        if use_phase_warp:
            ...
            n_vars = 5
        self.backbone = NBeatsBackbone(seq_len=seq_len, pred_len=pred_len, n_vars=n_vars, stack_count=stack_count, blocks_per_stack=blocks_per_stack, hidden_dim=hidden_dim, n_layers=n_layers, dropout=dropout)
...
def train_and_evaluate(label, use_phase_warp, train_loader, test_loader, device, y_std, y_mean, lat_size, lon_size, window, horizon, stack_count, blocks_per_stack, hidden_dim, n_layers, dropout, epochs, learning_rate, early_stopping_patience, base_dir):
    """
    训练一组 N-BEATS 实验，并返回评估指标。
    受控变量只有一个：是否启用 phase-warp 前端。
    """
    print(f'\n[Experiment] {label}')
    model = GridPointNBeatsO3(seq_len=window, pred_len=horizon, lat_size=lat_size, lon_size=lon_size, use_phase_warp=use_phase_warp, stack_count=stack_count, blocks_per_stack=blocks_per_stack, hidden_dim=hidden_dim, n_layers=n_layers, dropout=dropout).to(device)
...
def main():
    ...
    sys.stdout = Logger(os.path.join(base_dir, 'models', 'training_logs', 'NBEATS_PhaseWarp_Compare.txt'))
    ...
    for seed in RUN_SEEDS:
        ...
        raw_metrics = train_and_evaluate(label=f'NBEATS_Raw_seed{seed}', use_phase_warp=False, train_loader=train_loader, test_loader=test_loader, device=device, y_std=y_std, y_mean=y_mean, lat_size=lat_size, lon_size=lon_size, window=window, horizon=horizon, stack_count=stack_count, blocks_per_stack=blocks_per_stack, hidden_dim=hidden_dim, n_layers=n_layers, dropout=dropout, epochs=epochs, learning_rate=learning_rate, early_stopping_patience=early_stopping_patience, base_dir=base_dir)
        raw_seed_metrics.append(raw_metrics)
        set_experiment_seed(seed)
        phase_metrics = train_and_evaluate(label=f'NBEATS_PhaseWarp_seed{seed}', use_phase_warp=True, train_loader=train_loader, test_loader=test_loader, device=device, y_std=y_std, y_mean=y_mean, lat_size=lat_size, lon_size=lon_size, window=window, horizon=horizon, stack_count=stack_count, blocks_per_stack=blocks_per_stack, hidden_dim=hidden_dim, n_layers=n_layers, dropout=dropout, epochs=epochs, learning_rate=learning_rate, early_stopping_patience=early_stopping_patience, base_dir=base_dir)
    ...
    print('\n[Comparison Summary]')
    print_seed_summary('NBEATS_Raw', raw_summary)
    print_seed_summary('NBEATS_PhaseWarp', phase_summary)
